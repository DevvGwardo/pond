...
| [`pond deploy`](#pond-deploy) | Build and ship the capsule (local or to a hosted control plane) |
...
Build the capsule for production. Two modes, switched by whether you pass `--api`:

1. **Local deploy** — bundles `server/` and `client/` to `.pond/deploy-bundle.mjs` + `.pond/client.html`, writes `.pond/deploy.json`. Run with [`pond start`](#pond-start) to serve.
2. **Hosted deploy** — uploads the source to a control plane (`--api`). The control plane builds, boots a worker, and returns a public URL + a one-time `claimToken`.

**When to use:** shipping. `--api` for a hosted URL anyone can hit; no `--api` for a self-managed local bundle.

**Key flags:**
- `--api <url>` — control plane base URL (e.g. `https://pond.run`). Without it, deploy is local-only.
...
- `--port <n>` — port for local mode (default `3000`).

```sh
# Local build, run with `pond start`
pond deploy

# Anonymous hosted deploy — returns a URL + claimToken
pond deploy --api https://pond.run

# Authenticated hosted deploy, with env vars pushed
pond deploy --api https://pond.run --push-env
```

After a hosted deploy, the URL + `claimToken` are saved to `.pond/deploy.json`. The token is one-time — claim or rotate immediately.
...
- `--dir <name>` — override the local directory name (default: derived from the capsule title or deployId).
- `--api <url>` — control-plane base URL (auto-derived from the deploy URL if omitted).
- `--no-git` — skip `git init`.

```sh
pond fork https://abc.pond.run
pond fork https://abc.pond.run --dir my-copy
```
